...
NEWS_URL = "https://yh.wanmei.com/m/news/index.html"
...
class YHAdapter:
    ...
    def collect(self):
        ...
        preview_announced = bool(re.search(rf"{re.escape(next_version)}版本前瞻", news))
        preview_published = bool(
            re.search(rf"{re.escape(next_version)}版本前瞻特别节目", news)
            and "情报回顾" in news
        )
