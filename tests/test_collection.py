...
from main import collect_all, normalize, write_payload
...
class NormalizeTests(unittest.TestCase):
    def _raw(self, *, banner=None, sources=None):
        return {
            "name": "测试游戏",
            "current": {
                "version": "1.0",
                "start_at": "2026-09-01T11:00:00+08:00",
                "content": [],
            },
            "next": {
                "version": "1.1",
                "expected_start_at": "2026-10-01T11:00:00+08:00",
                "confirmed": False,
                "days_remaining": 1,
                "content": [],
            },
            ...
            "sources": sources or [
                {
                    ...
                    "checked_at": "2026-09-17T15:05:51+08:00",
                }
            ],
        }

    def test_sources_are_preserved_and_deduplicated(self):
        adapter = Mock(slug="test", name="测试游戏")
        raw = self._raw()
    ...
    def test_invalid_source_is_rejected(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "source title/url"):
            normalize(adapter, raw)

    def test_expired_banner_is_not_published_as_current(self):
        adapter = Mock(slug="test", name="测试游戏")
        raw = self._raw(banner={
            "characters": ["过期角色"],
            "start_at": "2020-01-01T00:00:00+08:00",
            "end_at": "2020-01-02T00:00:00+08:00",
        })
        result = normalize(adapter, raw)
        self.assertEqual(result["current_up_characters"], [])
        self.assertIsNone(result["current_up_start_at"])
        self.assertIsNone(result["current_up_end_at"])
        self.assertIsNone(result["current_up_days_remaining"])
